...
class PlantService(private val plantRepository: IPlantRepository) {
    ...
    suspend fun getAllPlantspc(call: ApplicationCall) {
        ...
        val plants = plantRepository.getPlantspc(search)
    ...
    suspend fun getPlantByIdpc(call: ApplicationCall) {
        ...
        val plant = plantRepository.getPlantByIdpc(id) ?: throw AppException(404, "Data komponen tidak tersedia!")
    ...
    // Ambil data request
    private suspend fun getPlantRequestpc(call: ApplicationCall): PlantRequest {
    ...
    private fun validatePlantRequestpc(plantReq: PlantRequest){
    ...
    suspend fun createPlantpc(call: ApplicationCall) {
        // Ambil data request
        val plantReq = getPlantRequestpc(call)

        // Validasi request
        validatePlantRequestpc(plantReq)

        // periksa plant dengan nama yang sama
        val existPlant = plantRepository.getPlantByNamepc(plantReq.nama)
        if(existPlant != null){
        ...
        val plantId = plantRepository.addPlantpc(
            plantReq.toEntity()
        )
    ...
    suspend fun updatePlantpc(call: ApplicationCall) {
        ...
        val oldPlant = plantRepository.getPlantByIdpc(id) ?: throw AppException(404, "Data komponen tidak tersedia!")
        ...
        val isUpdated = plantRepository.updatePlantpc(
    ...
    suspend fun deletePlantpc(call: ApplicationCall) {
        ...
        val isDeleted = plantRepository.removePlantpc(id)
        if (!isDeleted) {
    ...
    suspend fun getPlantImagepc(call: ApplicationCall) {
        ...
        val plant = plantRepository.getPlantByIdpc(id)
